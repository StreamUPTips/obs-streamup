...
## Improvements
- **Resize Spacers While You Edit Them** - Toolbar spacers used to be set once on creation. If you wanted a different size you had to delete it and add a new one. Now click a spacer in the configurator and the size box edits that spacer directly. The list draws it at its real width and the toolbar updates live as you change the number, so you can dial in the gap without guessing. Cancel puts everything back.
