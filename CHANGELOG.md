...
### New Features
- The OBS status bar readouts, on your toolbar. CPU, frame rate, missed frames, recording time, stream time, stream bitrate, recording bitrate and a status message are all items you drag onto the bar and order like anything else. Each one can show its label or just its value, and the two clocks can hold hours from the start instead of switching format when they pass an hour. They are read-only, one timer runs the lot at 1Hz, and each item is pinned to the width of the widest value it can ever show so the bar does not jitter every second while you stream. Side-dock the toolbar and they switch to a compact form, since 60.00 FPS does not fit a bar the width of a button. The messages are derived from OBS's own events rather than read off its status bar widget, which is private and would break without warning, so the wording is ours and will not match OBS word for word. Your OBS status bar is untouched, this puts the same numbers where you want them
