...
- Folders show an open or a closed folder icon depending on whether they are open, so you can tell at a glance without reading the arrow. A folder you have given an icon of your own keeps that icon in both states, since picking one on purpose and then having it change on you would be daft. The 2 icons are from Lucide, which we are free to ship, and they take the colour of your theme
...
- The Vertical Canvas control row is a proper toolbar now, when you are on the StreamUP theme. Aitum put those buttons in a plain row with nothing behind them, so there was no way for a theme to give them a background the way every other dock toolbar has one. StreamUP wraps the row so the theme can reach it, and adds dividers so the outputs, the clip controls and settings read as groups rather than one long line of icons. Nothing is moved or replaced, and on any other theme the dock is left exactly as Aitum built it
- The backtrack switch in the Vertical Canvas dock is one of the StreamUP theme's toggles now. It was there all along, it just had nowhere to be drawn: the button it lives in is 32px wide and the toggle is wider than that, so it was being cut away to nothing and looked like a missing icon. The button gives way instead
- The MultiDock body has rounded corners on the StreamUP theme, so the panel hugs the docks inside it instead of boxing them in
...
- The stream and record timers in the Vertical Canvas dock were cut off part way through, so a recording that had been going for hours showed you the first few digits and nothing else. Those 2 buttons are no longer given a width to stay inside, so they take whatever the timer needs
- The MultiDock toolbar sat taller than every other toolbar in the dock stack. It takes its height from OBS' own toolbar now, the same as the Scene Organiser toolbar does, so the 3 of them line up
- Moving a folder that had scenes selected inside it could leave a copy of those scenes behind, so the same scene turned up in the tree more than once and stayed there. A move puts the scenes in their new home and leaves it to OBS to take the originals away, which works fine for one row but not for a folder that has already rebuilt its own contents on the way past. The dock now checks the tree against its own record of which scene is which, after every change and before anything is saved, so a copy cannot survive. It tidies up a tree that already has some, so if you have seen this, one start sorts it
...
- Right click a status readout to reset it. Missed frames counts up for as long as OBS has been open, so a morning of dropped frames used to follow you into the evening's stream. Resetting leaves OBS's own counter alone and just counts from the moment you asked, and the status message can be dismissed early the same way. The clocks and the live readings do not offer it, since a recording time that can be zeroed is a recording time that lies
- The OBS status bar readouts, on your toolbar. CPU, frame rate, missed frames, recording time, stream time, stream bitrate, recording bitrate and a status message are all items you drag onto the bar and order like anything else. Each one is an icon and a value, the way the status bar reads, with the full name on hover. The icon can be turned off if you would rather have just the number, and the two clocks can hold hours from the start instead of changing shape when they pass an hour. They are read-only, one timer runs the lot at 1Hz, and each value is pinned to the width of the widest reading it can ever show so the bar does not jitter every second while you stream. Dock the toolbar down the side and every item stacks, icon above value, and switches to a shorter form, so the bar stays as narrow as it was. The message item takes no room at all until there is something to say. The messages are derived from OBS's own events rather than read off its status bar widget, which is private and would break without warning, so the wording is ours and will not match OBS word for word. Your OBS status bar is untouched, this puts the same numbers where you want them
...
- The toolbar background matches the OBS window behind it instead of sitting on it as a lighter panel
...
- The toolbar sits against the edge of the window it is docked to. It was floating 9px clear of it, which came from the padding StreamUP puts around the main window so your docks are not jammed into the corners. That padding is now dropped on whichever edge the toolbar is on, and put back when you move it
- The toolbar is one thickness wherever you dock it, and it hugs the buttons. There was 16px of chrome around them that never changed, so picking Small, Medium or Large barely moved anything, only 35px to 45px across the whole range. That is down to 6px, so the sizes read as actual sizes now
